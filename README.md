...
-   `auth_token`: A plain text file containing the `Bearer` token required for API authentication. **Note:** This file contains sensitive information and should not be shared or committed to version control.
...
2.  **Execution:** The project can be run using the Roc CLI. The command will execute the `main.roc` script.
    ...
    roc run main.roc
    ```
3.  **Output:** The application will print the full, raw JSON response from the API to the console. The URL and related parameters are configured in `main.roc`.
